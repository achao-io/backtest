...
def test_alts_offering_impact():
    ...
    try:
        ...
        print(f"Gap between news and market open: 2.5 hours")
        ...
        if len(premarket_bars) > 0:
            ...
            if pm_change_pct < -5:
                print(f"✅ STRONG PRE-MARKET REACTION: >5% decline before market open")
            elif pm_change_pct < -2:
                print(f"⚠️  MODERATE PRE-MARKET REACTION: >2% decline")
            elif pm_change_pct < 0:
                print(f"📉 MINOR PRE-MARKET DECLINE")
            else:
                print(f"📈 PRE-MARKET RISE (unexpected for offering news)")
        else:
